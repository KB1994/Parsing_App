...
def spark_AMBIMOB_ZTD(file_in_directory_input, abs_file_path, date_creation, programme, rules_file_path, soft_info):
...
######################################## Data frame Creation ################################################################################
    ...
    for file in file_in_directory_input:
        ...
        with open(text_file_path) as f:
            ...
            if(list_2G != []):
                list_data_frame_2G = Extract_Data_Frame_2G(list_2G, file, date_creation, programme, Date_Ref, "ZTD", band_Product_dict, base_bande_value)
                ...
                df_2G_table_finale.createOrReplaceTempView("Comp_Conf_avatar_2G")
                df_cell_tilt_2G = spark.sql("select cellname, bandfreq as bande from Comp_Conf_avatar_2G")
                #df_cell_tilt_initiale.union(df_cell_tilt_2G)
                list_cell_tilt.extend(df_2G_table_finale.select('cellname').rdd.flatMap(lambda x: x).collect())
            ...
            if(list_3G != []):
                list_data_frame_3G = Extract_Data_Frame_3G(list_3G, file, date_creation, programme, Date_Ref, "ZTD", band_Product_dict, base_bande_value)
                ...
                df_3G_table_finale .createOrReplaceTempView("Comp_Conf_avatar_3G")
                list_cell_tilt.extend(df_3G_table_finale.select('userlabel').rdd.flatMap(lambda x: x).collect())
                df_cell_tilt_3G = spark.sql("select cellname, case when left(cellname,1) = 'G' then '900' when left(cellname,1) = 'U' or left(cellname,1) = 'V' or left(cellname,1) = 'W' then '1800' end as bande from Comp_Conf_avatar_3G ")
                #df_cell_tilt_initiale.union(df_cell_tilt_3G)
            ...
            if(list_5G != []):
                list_data_frame_5G = Extract_Data_Frame_5G(list_5G, file, date_creation, programme, Date_Ref, "ZTD", gNBId, band_Product_dict, base_bande_value)
                ...
                pass
            if(list_4G != []):
                list_data_frame_4G = Extract_Data_Frame_4G(list_4G, file, info_pmRadioTxRankDistr_new , info_EUtranFreqRelation_new, date_creation, programme, Date_Ref, "ZTD", eNBId, band_Product_dict, base_bande_value)
            ...
            #print(missing_RET_Result)
            #print(missing_info_ret)
