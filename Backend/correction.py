...
def spark_AMBIMOB_CRZ(file_in_directory_input, abs_file_path, date_creation, programme, rules_file_path, soft_info):
   ...
   # zf.namelist()
    ...
    df_Export_Antenne_RET_elixir.Profondeur = df_Export_Antenne_RET_elixir.Profondeur.astype(str)
    df_Export_Antenne_RET_elixir.Longueur = df_Export_Antenne_RET_elixir.Longueur.astype(str)
    df_Export_Antenne_RET_elixir.Largeur = df_Export_Antenne_RET_elixir.Largeur.astype(str)
    ...
    df_Export_Antenne_RET_elixir_new = df_Export_Antenne_RET_elixir_new.withColumnRenamed("Nom Cellule", "Nom_Cellule")
...
######################################## Data frame Creation ################################################################################
    ...
    for file in file_in_directory_input:
        ...
        with open(text_file_path) as f:
            ...
            for el in info_EUtranFreqRelation:
                if el.startswith("EUtranCellFDD") and 'sCellCandidate' in el :
                    if el.split()[0].split(",")[1].split("=")[1] in ["227", "_251", "_103", "78"] and el.split()[0].split(",")[2].split("=")[1].startswith('P'):
                        pass
                    elif el.split()[0].split(",")[1].split("=")[1] in ['_6200', '_6300'] and el.split()[0].split(",")[2].split("=")[1].startswith('T'):
                        pass
                    elif el.split()[0].split(",")[1].split("=")[1] in ['_9385' , '_9235'] and el.split()[0].split(",")[2].split("=")[1].startswith('K'):
                        pass
                    elif el.split()[0].split(",")[1].split("=")[1] in ['_1850' , '_1501'] and el.split()[0].split(",")[2].split("=")[1].startswith('D'):
                        pass
                    elif el.split()[0].split(",")[1].split("=")[1] in ['_3175', '_2825'] and el.split()[0].split(",")[2].split("=")[1].startswith('L'):
                        pass
                    else:
            ...
            if(list_2G != []):
                list_data_frame_2G = Extract_Data_Frame_2G(list_2G, file, date_creation, programme, Date_Ref, "CRZ", band_Product_dict)
                list_data_frame_2G = insert_product(list_data_frame_2G, band_Product_RF)
                df_2G = spark.createDataFrame(pd.DataFrame(list_data_frame_2G))
                df_2G_final = df_2G.withColumnRenamed("Statut Tx x Rx x TsState x TsOper", "Statut_Tx_Rx_TsState_TsOper")
                df_2G_final.show()
                df_2G_final.write.jdbc(url="jdbc:postgresql://localhost:5432/qoc_django_db_new_db", table="int_info_2g", mode="append", properties={"user": "*", "password": "*", "driver":"org.postgresql.Driver"})
                df_2G_final.createOrReplaceTempView("int_info_2g")
                rru_2G_result = spark.sql(query_rru_2G)
                rru_2G_result.createOrReplaceTempView("rru_puissance_finale_2G")
                df_2G_table_finale = spark.sql(query_2G)
                df_2G_table_finale.write.jdbc(url="jdbc:postgresql://localhost:5432/qoc_django_db_new_db", table="comp_conf_avatar_2g", mode="append", properties={"user": "*", "password": "*", "driver":"org.postgresql.Driver"})
            if(list_3G != []):
                list_data_frame_3G = Extract_Data_Frame_3G(list_3G, file, date_creation, programme, Date_Ref, "CRZ", band_Product_dict)
                list_data_frame_3G = insert_product(list_data_frame_3G, band_Product_RF)
                df_3G = spark.createDataFrame(pd.DataFrame(list_data_frame_3G))
                df_3G_final = df_3G.withColumnRenamed("Statut Cell", "Statut_Cell")
                #df_3G_final.show()
                df_3G_final.write.jdbc(url="jdbc:postgresql://localhost:5432/qoc_django_db_new_db", table="int_info_3g", mode="append", properties={"user": "*", "password": "*", "driver":"org.postgresql.Driver"})
                df_3G_final.createOrReplaceTempView("int_info_3g")
                rru_3G_result = spark.sql(query_rru_3G)
                rru_3G_result.createOrReplaceTempView("rru_puissance_finale_3G")
                df_3G_table_finale = spark.sql(query_3G)
                df_3G_table_finale.write.jdbc(url="jdbc:postgresql://localhost:5432/qoc_django_db_new_db", table="comp_conf_avatar_3g", mode="append", properties={"user": "*", "password": "*", "driver":"org.postgresql.Driver"})
            if(list_5G != []):
                list_data_frame_5G = Extract_Data_Frame_5G(list_5G, file, date_creation, programme, Date_Ref, "CRZ", gNBId, band_Product_dict)
                ...
                pass
            if(list_4G != []):
                list_data_frame_4G = Extract_Data_Frame_4G(list_4G, file, info_pmRadioTxRankDistr_new , info_EUtranFreqRelation_new, date_creation, programme, Date_Ref, "CRZ", eNBId, band_Product_dict)
            ...
            print(list_noeud_to_delete_new)
            if len(list_noeud_to_delete_new) != 0:
                df_mainview_stat_finale = df_mainview_stat_finale.withColumn("Check_Noeud_A_Supprimer", "/".join(list_noeud_to_delete_new) + " : Noeud à supprimer")
...
def spark_BOLT_ZTD(file_in_directory_input, abs_file_path, date_creation, programme, rules_file_path, soft_info):
    #print(os.path.join(os.getcwd(), 'files/Conf_file/postgresql-42.2.12.jar'))
    spark = SparkSession.builder.appName("Python pyspark for postgresql").config("spark.jars", os.path.join(os.getcwd(), 'files/Conf_file/postgresql-42.2.12.jar')).master("local[4]").getOrCreate()
   ...
   # zf.namelist()
    ...
    df_Export_Antenne_RET_elixir_new = df_Export_Antenne_RET_elixir_new.withColumnRenamed("Nom Cellule", "Nom_Cellule")
    df_Export_Antenne_RET_elixir_new.createOrReplaceTempView("Antenne_RET_elixir")
    df_avatar_4G = spark.read.format("jdbc").option("url","jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1").option("dbtable","INT_AVATAR_BDI_4G").option("user","*").option("password","*").option("driver", "org.postgresql.Driver").load()
    df_avatar_4G = spark.read.csv(get_rules_file_path(rules_file_path, 'INT_AVATAR_BDI_4G'), header=True, sep=";", encoding="UTF8")
    #df_avatar_4G.show()
    #df_BDR_Erc_EutranCellFDD = spark.read.format("jdbc").option("url","jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1").option("dbtable","bdr_eutran_erc_eutrancelfdd").option("user","*").option("password","*").option("driver", "org.postgresql.Driver").load()
    df_BDR_Erc_EutranCellFDD = spark.read.csv(get_rules_file_path(rules_file_path, 'BDR_EUTRAN_Erc_EutranCellFDD'), header=True, sep=";", encoding="UTF8")
    df_avatar_5G = spark.read.format("jdbc").option("url","jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1").option("dbtable","INT_AVATAR_BDI_5G").option("user","*").option("password","*").option("driver", "org.postgresql.Driver").load()
    df_avatar_5G = spark.read.csv(get_rules_file_path(rules_file_path, 'INT_AVATAR_BDI_5G'), header=True, sep=";", encoding="UTF8")
    df_Optim_TACLAC = spark.read.format("jdbc").option("url","jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1").option("dbtable","Optim_TACLAC").option("user","*").option("password","*").option("driver", "org.postgresql.Driver").load()
    df_Optim_TACLAC = spark.read.csv(get_rules_file_path(rules_file_path, 'INT_OPTIM_TACLAC'), header=True, sep=";", encoding="UTF8")
    #df_Optim_TACLAC.show()
    df_rr_int_cellule = spark.read.format("jdbc").option("url","jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1").option("dbtable","rr_int_cellule").option("user","*").option("password","*").option("driver", "org.postgresql.Driver").load()
    ...
    #df_Export_Antenne_RET.show()
    df_int_optim_enodebid = spark.read.csv(get_rules_file_path(rules_file_path, 'INT_OPTIM_ENODEBID'), header=True, sep=";", encoding="UTF8")
    df_rru_puissance = spark.read.format("jdbc").option("url","jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1").option("dbtable","rru_puissance").option("user","*").option("password","*").option("driver", "org.postgresql.Driver").load()
    df_bdr_bdp_gnodeb = spark.read.csv(get_rules_file_path(rules_file_path, 'BDR_BDP_GNodeB'), header=True, sep=";", encoding="UTF8")
    Liste_ref_file.append(get_rules_file_path(rules_file_path, 'INT_AVATAR_BDI_4G'))
    Liste_ref_file.append(get_rules_file_path(rules_file_path, 'INT_AVATAR_BDI_5G'))
    Liste_ref_file.append(get_rules_file_path(rules_file_path, 'INT_OPTIM_TACLAC'))
...
######################################## Data frame Creation ################################################################################
    ...
    df_5G_final = spark.read.format("jdbc").option("url","jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1").option("dbtable","INT_INFO_5G").option("user","*").option("password","*").option("driver", "org.postgresql.Driver").load()
    df_4G_final = spark.read.format("jdbc").option("url","jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1").option("dbtable","INT_INFO_4G").option("user","*").option("password","*").option("driver", "org.postgresql.Driver").load()
    df_3G_final = spark.read.format("jdbc").option("url","jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1").option("dbtable","INT_INFO_3G").option("user","*").option("password","*").option("driver", "org.postgresql.Driver").load()
    df_2G_final = spark.read.format("jdbc").option("url","jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1").option("dbtable","INT_INFO_2G").option("user","*").option("password","*").option("driver", "org.postgresql.Driver").load()
    df_info_site = spark.read.format("jdbc").option("url","jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1").option("dbtable","INT_INFO_Site").option("user","*").option("password","*").option("driver", "org.postgresql.Driver").load()
    df_bdr_eutran_erc_sectorCarrier.createOrReplaceTempView("bdr_eutran_erc_sectorCarrier")
    df_bdr_ngran_erc_nrsectorCarrier.createOrReplaceTempView("bdr_ngran_erc_nrsectorCarrier")
    df_int_drim_interfaces_ip.createOrReplaceTempView("int_drim_interfaces_ip")
    df_bdr_utran_erc_nodeb.createOrReplaceTempView("bdr_utran_erc_nodeb")
    ...
    for file in file_in_directory_input:
        ...
        with open(text_file_path) as f:
            ...
            print(band_Product_dict)
            
            if(list_5G != []):
                list_data_frame_5G = Extract_Data_Frame_5G(list_5G, file, date_creation, programme, Date_Ref, "ZTD", gNBId, band_Product_dict)
                list_data_frame_5G = insert_product(list_data_frame_5G, band_Product_RF)
                df_5G = spark.createDataFrame(pd.DataFrame(list_data_frame_5G))
                df_5G_final = df_5G.withColumnRenamed("Statut Cell", "Statut_Cell")
                df_5G_final.createOrReplaceTempView("int_info_5g")
                df_5G_final.write.jdbc(url="jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1", table="int_info_5g", mode="append", properties={"user": "*", "password": "*", "driver":"org.postgresql.Driver"})
                df_avatar_5G.createOrReplaceTempView("int_avatar_bdi_5g")
                df_Optim_TACLAC.createOrReplaceTempView("optim_taclac")
                rru_result = spark.sql(query_rru)
                rru_result.createOrReplaceTempView("rru_puissance_finale")
                df_5G_table_finale = spark.sql(query_5G )
                df_5G_table_finale.createOrReplaceTempView("Comp_Conf_avatar_5G")
                df_5G_table_finale.write.jdbc(url="jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1", table="Comp_Conf_avatar_5G", mode="append", properties={"user": "*", "password": "*", "driver": "org.postgresql.Driver"})
                df_table_sythese =  spark.sql(query_1)
                df_table_sythese.write.jdbc(url="jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1", table="synthese_site", mode="append", properties={"user": "*", "password": "*", "driver": "org.postgresql.Driver"})
                #df_5G_table_finale.show()
                #df_5G_4G_Globale = spark.sql(query_5G_4G)
                #df_5G_4G_Globale.write.jdbc(url="jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1", table="comp_conf_avatar_4g_5g", mode="append", properties={"user": "*", "password": "*", "driver": "org.postgresql.Driver"})
            ...
            if(list_4G != []):
                #print(list_4G)
                list_data_frame_4G = Extract_Data_Frame_4G(list_4G, file, info_pmRadioTxRankDistr_new , info_EUtranFreqRelation_new, date_creation, programme, Date_Ref, "ZTD", eNBId, band_Product_dict)
                list_data_frame_4G = insert_product(list_data_frame_4G, band_Product_RF)
                df_4G = spark.createDataFrame(pd.DataFrame(list_data_frame_4G))
                df_4G_final = df_4G.withColumnRenamed("Statut Cell", "Statut_Cell")
                df_4G_final.write.jdbc(url="jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1", table="int_info_4g", mode="append", properties={"user": "*", "password": "*", "driver" : "org.postgresql.Driver"})
                df_4G_final.createOrReplaceTempView("int_info_4g")
                df_avatar_4G.createOrReplaceTempView("int_avatar_bdi_4g")
                df_Optim_TACLAC.createOrReplaceTempView("optim_taclac")
                rru_4G_result = spark.sql(query_rru_4G)
                ...
                df_4G_table_finale.write.jdbc(url="jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1", table="Comp_Conf_avatar_4G", mode="append", properties={"user": "*", "password": "*", "driver": "org.postgresql.Driver"})
                print("4G done")
                df_4G_table_finale.createOrReplaceTempView("Comp_Conf_avatar_4G")
                df_table_sythese =  spark.sql(query_2)
                df_table_sythese.write.jdbc(url="jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1", table="synthese_site", mode="append", properties={"user": "*", "password": "*", "driver": "org.postgresql.Driver"})
                #df_4G_5G_Globale = spark.sql(query_4G_5G)
                #df_4G_5G_Globale.write.jdbc(url="jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1", table="comp_conf_avatar_4g_5g", mode="append", properties={"user": "*", "password": "*", "driver": "org.postgresql.Driver"})
            ...
            #print("RET_Bloc_nok : ", RET_Bloc_nok)
            #print("RET_Bloc_connector : ", RET_Bloc_connector)
            list_data_cell_4G = extract_cell_Site_Cosite(list_4G, text_file_path_cosite['Cosite'],list_data_frame_4G, date_creation, programme, Date_Ref, "ZTD", band_Product_dict )
            list_band_mimo_4G = extract_band_mimo_4G(list_data_cell_4G)
            missing_info_ret = False
            dict_ret_state_finale = {}
            if(len(RET_Bloc) != 0 and len(RET_Bloc_connector) != 0):
            ...
            for con in RET_Bloc_connector:
                ...
                con_inter.append(con.split()[-1])
                result_info_ret_config_conectors.append(con_inter)
            #print(result_info_ret_config_conectors)
            result_rules_csv_text = parsing_data_for_csv_rules(all_rules_list,no_duplicates_name_antenna )
            result_logical_funct_for_MIMO_RET_RULES_values = check_MIMO_RET_RULES( info_MIMO_4x4_or_MIMO_2x2_site_cosite, result_rules_csv_text, result_info_ret_config_conectors)
            #print("result_logical_funct_for_MIMO_RET_RULES_values : ", result_logical_funct_for_MIMO_RET_RULES_values)
            if len(info_MIMO_4x4_or_MIMO_2x2_site_cosite) != 0:
            ...
            df_info_eran_finale_modif = df_info_eran_finale.join(df_mainview_stat_finale, df_info_eran_finale.SiteName == df_mainview_stat_finale.nename, 'left')
            df_info_eran_finale_modif.show()
            df_info_eran_finale_modif.write.jdbc(url="jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1", table="info_eran", mode="append", properties={"user": "*", "password": "*", "driver":"org.postgresql.Driver"})
            df_info_eran_finale.createOrReplaceTempView("info_eran")
            df_licence_alarm_ngs = spark.sql(query_licence_alarm_ngs)
            #df_licence_alarm_ngs.show()
            df_licence_alarm_ngs.write.jdbc(url="jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1", table="info_licence_alarm_ngs", mode="append", properties={"user": "*", "password": "*", "driver":"org.postgresql.Driver"})
            df_licence_alarm_ngs.createOrReplaceTempView("info_licence_alarm_ngs")
            df_mimo_handover_perf = spark.sql(query_mimo_handover_perf)
            #df_mimo_handover_perf.show()
            df_mimo_handover_perf.write.jdbc(url="jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1", table="info_mimo_ho_perf", mode="append", properties={"user": "*", "password": "*", "driver":"org.postgresql.Driver"})
            df_mimo_handover_perf.createOrReplaceTempView("info_mimo_ho_perf")
            df_info_4G_gen = spark.sql(query_info_4G_gen)
            #df_info_4G_gen.show()
            df_info_4G_gen.write.jdbc(url="jdbc:postgresql://localhost:5432/qoc_django_db_new_db_1", table="info_gen_4G", mode="append", properties={"user": "*", "password": "*", "driver":"org.postgresql.Driver"})
            #df_info_4G_gen.show()
            df_info_4G_gen.createOrReplaceTempView("info_gen_4g")
            print("info site done")
            df_table_sythese = spark.sql(query_3)
...
def spark_DI_ZTD(file_in_directory_input, abs_file_path, date_creation, programme, rules_file_path, soft_info):
    #print(os.path.join(os.getcwd(), 'files/Conf_file/postgresql-42.2.12.jar'))
    spark = SparkSession.builder.appName("Python pyspark for postgresql").config("spark.jars", os.path.join(os.getcwd(), 'files/Conf_file/postgresql-42.2.12.jar')).master("local[4]").getOrCreate()
...
######################################## Data frame Creation ################################################################################
    ...
    for file in file_in_directory_input:
        ...
        with open(text_file_path) as f:
            ...
            print(band_Product_dict)
            if(list_2G != []):
                list_data_frame_2G = Extract_Data_Frame_2G(list_2G, file, date_creation, programme, Date_Ref, "ZTD", band_Product_dict)
                list_data_frame_2G = insert_product(list_data_frame_2G, band_Product_RF)
                print("after 2G")
                df_2G = spark.createDataFrame(pd.DataFrame(list_data_frame_2G))
                df_2G_final = df_2G.withColumnRenamed("Statut Tx x Rx x TsState x TsOper", "Statut_Tx_Rx_TsState_TsOper")
                ...
                df_2G_table_finale.write.jdbc(url="jdbc:postgresql://localhost:5432/qoc_django_db_new_db", table="comp_conf_avatar_2g", mode="append", properties={"user": "*", "password": "*", "driver":"org.postgresql.Driver"})
            if(list_3G != []):
                list_data_frame_3G = Extract_Data_Frame_3G(list_3G, file, date_creation, programme, Date_Ref, "ZTD", band_Product_dict)
                list_data_frame_3G = insert_product(list_data_frame_3G, band_Product_RF)
                df_3G = spark.createDataFrame(pd.DataFrame(list_data_frame_3G))
                df_3G_final = df_3G.withColumnRenamed("Statut Cell", "Statut_Cell")
                df_3G_final.show()
...
def spark_DI_CRZ(file_in_directory_input, abs_file_path, date_creation, programme, rules_file_path, soft_info):
    #print(os.path.join(os.getcwd(), 'files/Conf_file/postgresql-42.2.12.jar'))
    spark = SparkSession.builder.appName("Python pyspark for postgresql").config("spark.jars", os.path.join(os.getcwd(), 'files/Conf_file/postgresql-42.2.12.jar')).master("local[4]").getOrCreate()
...
def spark_BOLT_CRZ(file_in_directory_input, abs_file_path, date_creation, programme, rules_file_path, soft_info):
   ...
   # zf.namelist()
    ...
    df_avatar_4G = spark.read.csv(get_rules_file_path(rules_file_path, 'INT_AVATAR_BDI_4G'), header=True, sep=";", encoding="UTF8")
    #df_avatar_4G.show()
    #df_BDR_Erc_EutranCellFDD = spark.read.format("jdbc").option("url","jdbc:postgresql://localhost:5432/qoc_django_db_new_db").option("dbtable","bdr_eutran_erc_eutrancelfdd").option("user","*").option("password","*").option("driver", "org.postgresql.Driver").load()
    df_BDR_Erc_EutranCellFDD = spark.read.csv(get_rules_file_path(rules_file_path, 'BDR_EUTRAN_Erc_EutranCellFDD'), header=True, sep=";", encoding="UTF8")
    df_avatar_5G = spark.read.format("jdbc").option("url","jdbc:postgresql://localhost:5432/qoc_django_db_new_db").option("dbtable","INT_AVATAR_BDI_5G").option("user","*").option("password","*").option("driver", "org.postgresql.Driver").load()
    df_avatar_5G = spark.read.csv(get_rules_file_path(rules_file_path, 'INT_AVATAR_BDI_5G'), header=True, sep=";", encoding="UTF8")
    df_Optim_TACLAC = spark.read.format("jdbc").option("url","jdbc:postgresql://localhost:5432/qoc_django_db_new_db").option("dbtable","Optim_TACLAC").option("user","*").option("password","*").option("driver", "org.postgresql.Driver").load()
            ...
            if (len(synchro_block) == 0):
                ...
                if extract_list_values_ngs !={}:
                    ...
                    if 'syncNodePriority' not in extract_list_values_ngs.keys() and 'syncRiPortStatus' not in extract_list_values_ngs.keys():
                        ...
                        dict_info_site_name['syncRiPortStatus'] = extract_list_values_ngs['syncRiPortStatus']
                        dict_info_site_name['check_synchronisation'] = 'OK'
                    elif extract_list_values_ngs['syncNodePriority'] == '3' or extract_list_values_ngs['syncNodePriority'] == '2' and 'OK_ACTIVE' in extract_list_values_ngs['syncRiPortStatus']:
                        dict_info_site_name['syncNodePriority'] = extract_list_values_ngs['syncNodePriority']
                        dict_info_site_name['syncRiPortStatus'] = extract_list_values_ngs['syncRiPortStatus']
                        dict_info_site_name['check_synchronisation'] = 'OK'
                    else:
...
def spark_Vercors(file_in_directory_input, abs_file_path, date_creation, programme, rules_file_path, soft_info):
    #print(os.path.join(os.getcwd(), 'files/Conf_file/postgresql-42.2.12.jar'))
    spark = SparkSession.builder.appName("Python pyspark for postgresql").config("spark.jars", os.path.join(os.getcwd(), 'files/Conf_file/postgresql-42.2.12.jar')).master("local[4]").getOrCreate()
...
from .Constants import  query_4G_5G, query_5G_4G, query_5G , query_4G, query_licence_alarm_ngs, query_mimo_handover_perf, query_info_eran, query_info_4G_gen, query_info_gen_4G_huw, query_1 , query_2, query_3, query_4, query_5, query_6, query_5G_TDD, query_1_tdd, query_3_tdd
...
######################################## Data frame Creation ################################################################################
    ...
    for file in file_in_directory_input:
        ...
        with open(text_file_path) as f:
            ...
            if(list_5G != []):
                list_data_frame_5G = Extract_Data_Frame_5G(list_5G, file, date_creation, programme, Date_Ref, "CRZ", gNBId, {})
                df_5G = spark.createDataFrame(pd.DataFrame(list_data_frame_5G))
                df_5G_final = df_5G.withColumnRenamed("Statut Cell", "Statut_Cell")
                ...
                df_5G_final.show()
                df_5G_final.write.jdbc(url="jdbc:postgresql://localhost:5432/qoc_django_db_new_db", table="int_info_5g_tdd", mode="append", properties={"user": "*", "password": "*", "driver":"org.postgresql.Driver"})
            ...
            for el in Activation_Compteur_block:
                if el.startswith("/ericsson"):
                    ...
                    list_info2.append(el)
            if("5G" in "".join(list_info2) and len(list_5G) >= 1 and 'Pas de cellules 5G' not in ''.join(a_info_cell_sectors)):
                if(len(list_info1) == 0):
                    dict_info_site_name['Activation_Compteur'] = "Absence XML"
                    dict_info_site_name['Check_Activation_Compteur'] = "NOK"
                ...
                if("5G" in "".join(list_info2) and 'Pas de cellules 5G' in ''.join(a_info_cell_sectors)):
                    if(len(list_info1) == 0):
                        dict_info_site_name['Activation_Compteur'] = "Absence XML"
                        dict_info_site_name['Check_Activation_Compteur'] = "NOK"
                    else:
                    ...
                    if("5G" not in "".join(list_info2) and 'Pas de cellules 5G' not in ''.join(a_info_cell_sectors)):
                        dict_info_site_name['Activation_Compteur'] = "Cellule 5G existe/Absence Compteur 5G"
                        dict_info_site_name['Check_Activation_Compteur'] = "NOK"
                    elif ("5G" in "".join(list_info2) and 'Pas de cellules 5G' not in ''.join(a_info_cell_sectors)):
                        dict_info_site_name['Activation_Compteur'] = "Absence Cellule 5G/Compteur 5G existe"
                        dict_info_site_name['Check_Activation_Compteur'] = "NOK"
            ...
            df_info_site_tdd.write.jdbc(url="jdbc:postgresql://localhost:5432/qoc_django_db_new_db", table="int_info_site_tdd", mode="append", properties={"user": "*", "password": "*", "driver":"org.postgresql.Driver"})
            df_info_site_tdd.createOrReplaceTempView("int_info_site_tdd")
