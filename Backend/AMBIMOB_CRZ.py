...
def spark_AMBIMOB_CRZ(file_in_directory_input, abs_file_path, date_creation, programme, rules_file_path, soft_info):
   ...
   # zf.namelist()
    ...
    #spark.read.csv(os.path.join(os.getcwd(), '/opt/application/*/Requete_QoC_Tarek_41_6775481267087365322.xlsx'), header=True, sep=";", encoding="UTF8")
...
######################################## Data frame Creation ################################################################################
    ...
    for file in file_in_directory_input:
        ...
        with open(text_file_path) as f:
            ...
            if(list_2G != []):
                list_data_frame_2G = Extract_Data_Frame_2G(list_2G, file, date_creation, programme, Date_Ref, "CRZ", band_Product_dict, base_bande_value)
            ...
            if(list_3G != []):
                list_data_frame_3G = Extract_Data_Frame_3G(list_3G, file, date_creation, programme, Date_Ref, "CRZ", band_Product_dict, base_bande_value)
            ...
            for el in e_info_soft:
                if el.startswith('Node: RadioNode'):
                    ...
                    for soft in soft_info.keys():
                        #print(soft_info[soft])
                        #print(soft_info[soft] in ''.join(el.split()))
                        #print(soft.strip())
                        #print('BB' + dict_info_site_name["BaseBand"])
                        #print('BB' + dict_info_site_name["BaseBand"].strip() in soft.strip())
